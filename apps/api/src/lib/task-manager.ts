...
export type RequestType =
  ...
  | 'action_request';
export type HandoffExecutionMode =
  | 'autonomous_code_change'
  | 'context_graph_answer'
  | 'app_action';
export type HandoffFinalResponseStyle =
  | 'pull_request_only'
  | 'succinct_answer'
...
export interface PromptHandoffPlan {
  ...
  finalResponseStyle: HandoffFinalResponseStyle;
  inferredIntent: string;
  inferredDeliverable: string;
  screenshotSummary: string;
  contextSources: HandoffContextSource[];
...
const buildDefaultHandoffPlan = (): PromptHandoffPlan => ({
  ...
  inferredIntent: 'Answer the user using the stored request context.',
  inferredDeliverable: 'A short answer for the current bubble request.',
  screenshotSummary: 'No screenshot understanding was generated by the fallback processor.',
  contextSources: ['screen'],
  suggestedSkills: [],
  targetRepoId: null,
  expandedPrompt:
    'Answer the request from the stored prompt and screenshot metadata. Keep the reply short.'
});
